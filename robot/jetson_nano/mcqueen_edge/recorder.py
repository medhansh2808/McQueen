"""Jetson-local canonical RGB/action dataset recorder for McQueen.

Runs inside mcqueen-edge so it shares the same DriveController state as the
physical actuators and never competes for UDP 5007 or HTTP 8080.
Compatible with Python 3.6+ on the Jetson Nano image.
"""
...
class JetsonDatasetRecorder(threading.Thread):
    """Capture Lenovo RGB and the exact current Jetson actuator command."""
    ...
    def _start_episode(self):
        ...
        self.frames_file = (self.episode_dir / "frames.jsonl").open(
            "w",
            buffering=1,
        )
    ...
    def _save_frame(self, frame, frame_timestamp):
        drive_state = self.drive.snapshot()

        servo_angle = steering_to_angle(drive_state["steering"])
        ...
        row = {
            ...
            "mcqueen.raw.steering_command": int(
                drive_state["steering"]
            ),
            "mcqueen.raw.throttle_command": int(
                drive_state["throttle"]
            ),
            "mcqueen.raw.motor_enabled": bool(
                drive_state["motor_enabled"]
            ),
        }
        ...
        if self.frame_index == 1 or self.frame_index % 10 == 0:
            print(
                "Recorder frame={:05d} servo={:3d} pwm={:4d}".format(
                    self.frame_index,
                    int(servo_angle),
                    int(motor_pwm),
                ),
    ...
    def run(self):
        ...
        try:
            while not self.stop_event.is_set():
                ...
                requested = bool(
                    self.runtime_state.snapshot().get("recording", False)
                )
                ...
                while self.next_save_monotonic <= now:
                    self.next_save_monotonic += 1.0 / self.record_fps

                if (
                    self.last_frame_timestamp is not None
                    and frame_timestamp <= self.last_frame_timestamp
                ):
                    continue

                self.last_frame_timestamp = frame_timestamp
                self._save_frame(frame, frame_timestamp)

        except Exception as exc:
            print(
                "ERROR: Jetson dataset recorder crashed: {!r}".format(exc),
                flush=True,
            )
