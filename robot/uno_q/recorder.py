...
class EpisodeWriter:
    """Lightweight on-device spool that converts deterministically to LeRobotDataset v3."""
    ...
    def _write_episode_json(self, status: str, **extra: object) -> None:
        ...
        payload: dict[str, object] = {
            "schema_version": "mcqueen-spool-v1",
    ...
    def save(
        ...
        with self.lock:
            ...
            depth_name = f"frame_{index:06d}.png"
            rgb_path = self.rgb_dir / rgb_name
            depth_path = self.depth_dir / depth_name

            rgb_tmp = rgb_path.with_suffix(".jpg.tmp")
            depth_tmp = depth_path.with_suffix(".png.tmp")
            rgb_tmp.write_bytes(clean_jpeg)
            Image.fromarray(depth_mm.astype(np.uint16, copy=False)).save(
                depth_tmp, format="PNG", compress_level=1
            )
            ...
            row = {
                ...
                "observation.images.front_depth": f"depth/{depth_name}",
                "action.steering_command": int(steering_command),
                "action.throttle_command": int(throttle_command),
