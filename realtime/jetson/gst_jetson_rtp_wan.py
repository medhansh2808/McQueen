...
Transport design (lab-verified path, 2026-08-13):
  ...
  - appsrc -> videoconvert -> nvvidconv (NVMM NV12) -> NVENC H.264 ->
    ...
    pattern from realtime/bench/direct_udp_peer.py (60/60 ACK proof,
...
class Sender(object):
    def __init__(self, args):
        ...
        self.appsrc = None
        self.frame_id = 0
        self.hold_s = 0.0            # hysteresis turn-hold: last committed steering
        self.straight_count = 0    # consecutive near-straight frames
        self._steer_gain = 3.0       # authority boost: model near-center -> real turn
        self._deadband = 0.12        # below this = model "near straight"
        self._straight_hold_frames = 12  # straight frames before easing (~0.4s)
        self._return_alpha = 0.80    # ease-to-center speed once confirmed straight
    ...
    def ctrl_loop(self):
        edge = None
        if self.args.ctrl_to_edge:
            h, p = self.args.ctrl_to_edge.rsplit(":", 1)
            edge = (h, int(p))
            esock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            # arm the edge session with neutrals first (gate requirement)
            for i in range(15):
                ts = int(time.time() * 1000) % 100000000
                esock.sendto("C,rc-car,{},{},{},0,0,0\n".format(
                    self.args.edge_session, i, ts).encode(), edge)
                time.sleep(0.05)
            print("[JETSON-CAM] edge session armed -> {}".format(
                self.args.ctrl_to_edge), flush=True)
        eseq = 100
        while not self.stop_event.is_set():
            try:
            ...
            if edge is not None:
                eseq += 1
                deg = float(msg.get("servo_angle_deg", 90.0))
                pwm255 = int(msg.get("motor_pwm", 0))
                # 2026-08-23 lab fix: physical linkage inverts pulse direction.
                # Model RIGHT (deg<90) must map to NEGATIVE raw steering so the
                # backend pulses toward servo_left_us (1750), which physically
                # turns RIGHT. Flip applied here (verified empirically at lab).
                s = -(90.0 - deg) / 25.0 if deg >= 90 else -(90.0 - deg) / 45.0
                # 2026-08-23 lab fix #6: hysteresis turn-hold. The model under
                # loop latency oscillates right->center->right. A single
                # near-straight frame must NOT drop the turn. We hold the last
                # committed turn and only ease toward center after N consecutive
                # straight frames (~0.4s), which is a real "straight" signal.
                s = s * self._steer_gain
                s = max(-1.0, min(1.0, s))
                if abs(s) >= self._deadband:
                    self.hold_s = s
                    self.straight_count = 0
                else:
                    self.straight_count += 1
                    if self.straight_count >= self._straight_hold_frames:
                        self.hold_s *= self._return_alpha   # confirmed straight
                        if abs(self.hold_s) < 0.03:
                            self.hold_s = 0.0
                s = self.hold_s

                # 2026-08-26: model-controlled throttle. The 2026-08-23
                # constant-full-throttle mandate is retired -- throttle now
                # tracks the policy head output again. Steering tuning below
                # (flip / gain / hysteresis) is hardware-verified, unchanged.
                t = max(-1.0, min(1.0, pwm255 / 255.0))
                duty = max(0.0, min(1.0, abs(t)))
                ts = int(time.time() * 1000) % 100000000
                raw_s = int(round(s * 1000))
                raw_t = int(round(duty * 1000))
                mo = 1 if duty > 0.01 else 0
                try:
                    esock.sendto("C,rc-car,{},{},{},{},{},{}\n".format(
                        self.args.edge_session, eseq, ts, raw_s, raw_t, mo
                    ).encode(), edge)
                except Exception as exc:
                    print("[JETSON-CAM] EDGE send error {}".format(exc), flush=True)
    ...
    def capture_loop(self):
        ...
        if self.args.fake_source:
            dev = self.args.fake_source
            print("[JETSON-CAM] FAKE-SOURCE synthetic bench: {}".format(dev),
                  flush=True)
        ...
        while not self.stop_event.is_set():
            ...
            if not ok or frame is None:
                if self.args.fake_source:
                    # synthetic bench: loop the clip forever
    ...
    def build(self):
        ...
        if self.args.encoder:
            enc = self.args.encoder.format(kbps=self.args.bitrate_kbps * 1000)
        elif self.args.fake_source:
            # Synthetic bench off-Jetson: SW x264 (RTX host lacks it → pass
            # --encoder 'nvh264enc bitrate={kbps} key-int-max=30 ! h264parse
            # name=parse config-interval=-1' when benching ON the RTX).
            enc = ("x264enc tune=zerolatency bitrate={} speed-preset=ultrafast "
                   "key-int-max=30 ! h264parse name=parse "
                   "config-interval=-1").format(self.args.bitrate_kbps)
        else:
            enc = ("x264enc tune=zerolatency bitrate={} speed-preset=ultrafast "
                   "key-int-max=30 ! h264parse name=parse "
                   "config-interval=-1").format(self.args.bitrate_kbps)
...
def main():
    ...
    p.add_argument("--ctrl-to-edge", default=None,
                   help="ip:port of mcqueen-edge :5007 — forward CTRL as C-packets (actuation)")
    p.add_argument("--edge-session", default="autopilot")
    ...
    p.add_argument("--encoder", default=None,
                   help="override encoder element string ({kbps} placeholder)")
    p.add_argument("--fake-source", default=None,
                   help="video/image-sequence path for a SYNTHETIC sender "
                        "(full-loop bench without hardware; 2026-08-22)")
    args = p.parse_args()
    Sender(args).run()
