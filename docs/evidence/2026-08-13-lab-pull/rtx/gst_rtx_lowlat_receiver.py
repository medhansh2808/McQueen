...
class Receiver(object):
    ...
    def on_sample(self, sink):
        ...
        # Prove the real decoded appsink buffer can enter CUDA/PyTorch and
        # produce steering/throttle values. This is deliberately a tiny dummy
        # policy, not the final learned driver.
        if self.frames % 5 == 0:
            try:
                ...
                buf_for_ai = sample.get_buffer()
                ok, mapinfo = buf_for_ai.map(Gst.MapFlags.READ)
                if ok:
                    try:
                        ...
                        # Dummy action head bounded to project-safe dry-run values.
                        servo = 90.0 + max(-5.0, min(5.0, (mean - 0.5) * 10.0))
                        pwm = 0
                        if self.torch.cuda.is_available():
                            self.torch.cuda.synchronize()
                        dt = (time.perf_counter() - t0) * 1000.0
                        self.infer_ms.append(dt)
                        self.infer_count += 1
                        if self.infer_count % 10 == 0:
                            tail = self.infer_ms[-20:]
                            print(
                                "[RTX-GST] DUMMY_INFER count={} last={:.2f}ms avg20={:.2f}ms action=({:.1f},{})".format(
                                    self.infer_count, dt, sum(tail) / len(tail), servo, pwm
                                ),
                                flush=True,
                            )
                    finally:
                        buf_for_ai.unmap(mapinfo)
            except Exception as exc:
                print("[RTX-GST] DUMMY_INFER ERROR {}".format(exc), flush=True)

        # Receiver-local freshness metric. This is not cross-machine clock math:
        # both buffer PTS and running time are interpreted on this RTX pipeline.
        recv_age_ms = None
        try:
